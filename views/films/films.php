...
<?php 
  if(isset($_POST["filter"])&&($_POST["search"] !== "" || $_POST["genres"] !== "")){
    ...
    foreach($allFilms as $film){
      ...
      if(strpos(strtolower($film -> title), strtolower($search)) !== false || $search == ""){
        $searchOk = true;
      }
      if(in_array($genre, $film -> genres)== True || $genre == ""){
          $genreOk = true;
      }
      if($searchOk && $genreOk){
...
<div class="uk-margin-left uk-margin-large-left uk-margin-large-right uk-margin-bottom">
  <h1 class="uk-font uk-text-center">Films</h1>
        ...
        <div class="image-fond">
          <div class="uk-card uk-card-default uk-raduis contenu" style="position: relative;">
            <div class="image-crop">
              <img src="<?=$film->image_url ?>" alt="<?=$film->title ?>" width="100%" class="uk-raduis-img uk-img">
            </div>
            <div class="uk-margin-left uk-margin-right" style="margin-bottom: 70px;">
              <h2 class="uk-card-title uk-text-center uk-active uk-font"><?=$film->title ?></h2>
              <?php 
                $mots = explode(" ", $film->synopsis);
            ...
            <a href="filmDetail.php?id=<?=$film->id ?>" class="uk-link-reset uk-position-bottom-center uk-margin-bottom">
