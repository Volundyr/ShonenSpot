...
<?php 
  if(isset($_POST["filter"])&&($_POST["search"] !== "" || $_POST["genres"] !== "")){
    ...
    foreach($allMangas as $manga){
      ...
      if(strpos(strtolower($manga -> title), strtolower($search)) !== false || $search == ""){
        $searchOk = true;
      }
      if(in_array($genre, $manga -> genres)== True || $genre == ""){
          $genreOk = true;
      }
      if($searchOk && $genreOk){
...
<div class="uk-margin-left uk-margin-large-left uk-margin-large-right uk-margin-bottom">
  <h1 class="uk-font uk-text-center">Mangas</h1>
  ...
  ?>
        <h2 class="uk-font uk-active uk-text-center">Désolé il n'y a pas d'manga appelé <?=$_POST['search']?><?=$genre?>.</h2>
  <?php 
      }else{
  ?>
        ...
        <div class="image-fond">
          <div class="uk-card uk-card-default uk-raduis contenu" style="position: relative;">
            <div class="image-crop">
              <img src="<?=$manga->image_url ?>" alt="<?=$manga->title ?>" width="100%" class="uk-raduis-img uk-img">
            </div>
            <div class="uk-margin-left uk-margin-right" style="margin-bottom: 70px;">
              <h2 class="uk-card-title uk-text-center uk-active uk-font"><?=$manga->title ?></h2>
              <?php 
            ...
            <a href="mangaDetail.php?id=<?=$manga->id ?>" class="uk-link-reset uk-position-bottom-center uk-margin-bottom">
